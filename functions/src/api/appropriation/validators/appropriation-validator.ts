...
class AppropriationCreateValidator {
  static async handle(req: Request, _: Response, next: NextFunction) {
    const schema = Joi.object({
      data: Joi.object({
        appropriationNumber: Joi.string().min(1).required(),
    ...
    if (validated.error)
      throw new BadRequest(
        ...
        "AppropriationCreateValidator.handle"
      );
...
export default AppropriationCreateValidator;
